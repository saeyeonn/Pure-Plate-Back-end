...
def restaurants_in_categories_view(request):
    ...
    try:
        ...
        categories = Category.objects.filter(category_name__in=category_names_list)
        ...
        for category_name in category_names_list:
            category_restaurants = restaurants.filter(categories__category_name=category_name)
            category_list = [{
